...
Cell::Cell(object &o) : x(o["x"].get<double>()),
                        y(o["y"].get<double>()) {}
Cell::Cell(const Cell &original) : x(original.x), y(original.y) {}
...
Agent::Agent(int x, int y, int d) : Cell(x, y), direction(d) {}
Agent::Agent(object &o) : Cell(o), direction(o["direction"].get<double>()) {}
Agent::Agent(const Agent &original) : Cell(original), direction(original.direction) {}

Gold::Gold(int x, int y, int a) : Cell(x, y), amount(a) {}
...
Field::Field(object &json)
{
  ...
  int i = 0;
  for (auto &a : agts)
    agents[i++] = Agent(a.get<object>());
  auto &hls = json["holes"].get<value::array>();
  for (auto &h : hls)
    holes.push_back(Cell(h.get<object>()));
  auto &kg = json["known"].get<value::array>();
  for (auto &g : kg)
    known.push_back(Gold(g.get<object>()));
  auto &hg = json["hidden"].get<value::array>();
  for (auto &g : hg)
    hidden.push_back(Gold(g.get<object>()));
...
Field::Field(const Field &fld)
{
  size = fld.size;
  int i = 0;
  for (auto a : fld.agents)
    agents[i++] = Agent(a);
  for (auto h : fld.holes)
    holes.push_back(Cell(h));
  for (auto kg : fld.known)
    known.push_back(Gold(kg));
  for (auto hg : fld.hidden)
    hidden.push_back(Gold(hg));
        ...
        if (a >= 2) { // Dog barks on a treasure
          auto found =
              find_if(hidden.begin(), hidden.end(),
                      [target](auto &g) { return g.x == target.x && g.y == target.y; });
          if (found != hidden.end()) {

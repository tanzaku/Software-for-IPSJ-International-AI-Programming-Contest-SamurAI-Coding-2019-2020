...
static void sendGameInfo(FILE *out, int id, int step,
                         ...
                         int timeLeft)
{
  ...
  for (auto h : l.holes)
    fprintf(out, " %d %d", h.x, h.y);
  fprintf(out, "\n");
  fprintf(out, "%d", (int)l.known.size());
  for (auto g : l.known)
    fprintf(out, " %d %d %d", g.x, g.y, g.amount);
  fprintf(out, "\n");
  int x = l.agents[id].x;
  int y = l.agents[id].y;
  vector<Gold> sensed;
...
vector<StepLog> playGame(const Configuration &initialConf,
                         char *scripts[], char *dumpPath, int numScripts)
{
  const Configuration *config = &initialConf;
  int coordWidth = config->size > 10 ? 2 : 1;
  int totalGolds = 0;
  for (auto g : initialConf.known)
    totalGolds += g.amount;
  for (auto g : initialConf.hidden)
    totalGolds += g.amount;
  FILE *dump[4];
  if (dumpPath != nullptr) {
    for (int p = 0; p != 4; p++) {
  ...
  for (int p = 0; p != 4; p++) {
    ...
    playerIds[p] = pid;
    close(pipeOut[0]);
    toPlayers[p] = fdopen(pipeOut[1], "w");
    close(pipeIn[1]);
    fromPlayers[p] = fdopen(pipeIn[0], "r");
  ...
  for (int step = 0; step < config->steps; step++) {
    ...
    for (int p = 0; p != 4; p++) {
      ...
      thread receiver([](FILE *in, int &plan, bool &done,
                         steady_clock::time_point &receivedAt) {
        fscanf(in, "%d", &plan);
    ...
    if (stepSummary) {
      for (int p = 0; p != 4; p++) {
        ...
        cerr << (plans[p] < 0 ? "stay" : plans[p] < 8 ? "move" : plans[p] < 16 ? "dig " : "plug")
